...
🖥️ **Relay Station Agent Control (macOS)** — Tray submenu for Relay Station pipeline shortcuts, including a native Claude Code background-session launch; see [RELAY_AGENT_CONTROL.md](RELAY_AGENT_CONTROL.md)
...
Right-click the tray icon for: Show/Hide, Refresh, Re-login, Settings, Exit. On macOS, a **RELAY STATION // AGENT CONTROL** submenu also appears; see [RELAY_AGENT_CONTROL.md](RELAY_AGENT_CONTROL.md).
